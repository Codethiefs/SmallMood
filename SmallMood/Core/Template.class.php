...
class  Template
{
    ...
    {
        $this->config['cache_path'] = Config::config('CACHE_PATH');
        $this->config['template_suffix'] = Config::config('TMPL_TEMPLATE_SUFFIX');
        $this->config['cache_suffix'] = Config::config('TMPL_CACHFILE_SUFFIX');
        $this->config['tmpl_cache'] = Config::config('TMPL_CACHE_ON');
        $this->config['cache_time'] = Config::config('TMPL_CACHE_TIME');
        $this->config['taglib_begin'] = $this->stripPreg(Config::config('TAGLIB_BEGIN'));
        $this->config['taglib_end'] = $this->stripPreg(Config::config('TAGLIB_END'));
        $this->config['tmpl_begin'] = $this->stripPreg(Config::config('TMPL_L_DELIM'));
        $this->config['tmpl_end'] = $this->stripPreg(Config::config('TMPL_R_DELIM'));
        $this->config['default_tmpl'] = Config::config('TEMPLATE_NAME');
        $this->config['layout_item'] = Config::config('TMPL_LAYOUT_ITEM');
    }
    ...
    public function fetch($prefix = '')
    {
        ...
        // 生成缓存文件位置
        $tplCacheFile = $this->config['cache_path'] . $prefix . md5($tplFile) . $this->config['cache_suffix'];
        echo $tplCacheFile;
        // 编译模板内容
        $tplContent = $this->compiler($this->tplContent);
        Storage::getInstance()->put($tplCacheFile, trim($tplContent), 'tpl');
    ...
    protected function compiler($tplContent)
    {
        ...
        // 还原被替换的Literal标签
        $tplContent = preg_replace_callback('/<!--###literal(\d+)###-->/is', array($this, 'restoreLiteral'), $tplContent);
        // 添加安全代码
